...
struct pblk_line {
	...
	atomic_long_t line_encode_time;	/* Blocks left for sync erasing */
	...
	unsigned long wom_total_invalid_cells; /* total number of invalid cells in each line */
	unsigned long wom_gc; 		/* total number of GCs in this line */
	int wom_zero_data; 		/* 0 or 1, if 0 -> prev data should be zeroed. 
					if 1, previous data should not be zeroed. */
...
struct pblk {
	...
	sector_t p_capacity; /* Device capacity without overprovisioning */
	...
	unsigned char *trans_reverse_map;
						...
						struct ppa_addr ppa)
{
	if (pblk->addrf_len < 32) {
		u32 *map = (u32 *)pblk->trans_map;
	#ifdef DEBUG_PRINTS
		printk("%s():lba %llu ppa %u\n", __func__, lba, pblk_ppa64_to_ppa32(pblk, ppa));
	#endif
...
static inline sector_t pblk_trans_reverse_map_get(struct pblk *pblk, struct ppa_addr ppa)
{
	if (pblk->addrf_len < 32) {
		u32 *reverse_map = (u32 *)pblk->trans_reverse_map;

		uint32_t ppa_32 = pblk_ppa64_to_ppa32(pblk, ppa);
		if(ppa_32 < (pblk->p_capacity  ) ) {
			return reverse_map[ppa_32];
		}else {
			printk("%s(): XXX PPA %u CAPACITY %llu REVERSE_MAP_CAPACITY %llu\n", __func__, ppa_32, pblk->capacity, pblk->capacity * (100 + pblk->op) / 100);
			return ADDR_EMPTY;
		}
	} else {
		u64 *reverse_map = (u64 *)pblk->trans_reverse_map;
		return reverse_map[ppa.ppa];
	}
...
static inline void pblk_trans_reverse_map_set(struct pblk *pblk, struct ppa_addr ppa,
	sector_t lba)
{
	if (pblk->addrf_len < 32) {
		u32 *reverse_map = (u32 *)pblk->trans_reverse_map;
		...
		uint32_t ppa_32 = pblk_ppa64_to_ppa32(pblk, ppa);
		if(ppa_32 < (pblk->p_capacity  ) ) {
			if(reverse_map[ppa_32] != ADDR_EMPTY && reverse_map[ppa_32] != 4294967295 ) {
			#ifdef DEBUG_PRINTS
				printk("%s():PPA %u LBA update %u to %llu\n", __func__, ppa_32, reverse_map[ppa_32], lba);
			#endif
			}
			reverse_map[ppa_32] = lba;
		}else {
			#ifdef DEBUG_PRINTS
			printk("%s(): XXX PPA %u CAPACITY %llu REVERSE_MAP_CAPACITY %llu\n", __func__, ppa_32, pblk->capacity, pblk->capacity * (100 + pblk->op) / 100);
			#endif
		...
		u64 *reverse_map = (u64 *)pblk->trans_reverse_map;
		if(reverse_map[ppa.ppa] != ADDR_EMPTY) {
			#ifdef DEBUG_PRINTS
			printk("%s():update %llu to %llu\n", __func__, reverse_map[ppa.ppa], lba);
			#endif
		}
		reverse_map[ppa.ppa] = lba;
	}
}
