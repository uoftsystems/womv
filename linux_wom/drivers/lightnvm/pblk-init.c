...
static size_t pblk_trans_map_size(struct pblk *pblk)
{
	...
	printk("%s():trans map size is %llu\n", __func__, entry_size * pblk->capacity);

	return entry_size * pblk->capacity;
}

static size_t pblk_trans_reverse_map_size(struct pblk *pblk)
{
	int entry_size = 8;
	...
	printk("%s():trans reverse map size is %llu\n", __func__, pblk->p_capacity * entry_size);
	return (entry_size * pblk->p_capacity);
}
...
static void pblk_p2l_free(struct pblk *pblk)
{
	vfree(pblk->trans_reverse_map);
}
...
static int pblk_l2p_init(struct pblk *pblk, bool factory_init)
{
	...
	printk("%s():pblk_trans_map_size = %lu\n", __func__, map_size);
	pblk_ppa_set_empty(&ppa);

	for (i = 0; i < pblk->capacity; i++) {
		printk("i %llu capacity %llu\n", i, pblk->capacity);
...
static int pblk_p2l_init(struct pblk *pblk)
{
	sector_t i;
	struct ppa_addr ppa;
	size_t map_size;
	...
	map_size = pblk_trans_reverse_map_size(pblk);
	pblk->trans_reverse_map = __vmalloc(map_size, GFP_KERNEL | __GFP_NOWARN
					| __GFP_RETRY_MAYFAIL | __GFP_HIGHMEM,
					PAGE_KERNEL);
	if (!pblk->trans_reverse_map) {
		pblk_err(pblk, "failed to allocate P2L Map (need %zu of memory)\n",
				map_size);
		return -ENOMEM;
	}

	printk("%s():pblk_reverse_trans_map_size = %lu\n", __func__, map_size);

	for (i = 0; i < map_size ; i++) {
		ppa.ppa = i;
		pblk_trans_reverse_map_set(pblk, ppa, ADDR_EMPTY);
	}
...
static int pblk_set_provision(struct pblk *pblk, int nr_free_chks)
{
	...
	if ((nr_free_chks - provisioned) < minimum) {
		...
		printk("%s():provisioned---xx %llu nr_free_chks %d minimum %d\n", __func__, provisioned, nr_free_chks, minimum);
	...
	printk("geo clba = %d\n", geo->clba);
	printk("min write pgs = %d\n", geo->clba);
	printk("min write pgs data = %d\n", pblk->min_write_pgs_data);
	printk("clba = %d\n", clba);

	pblk->capacity = (provisioned - blk_meta) * clba;
	printk("%s():nr_free_chks = %d\n", __func__, nr_free_chks);
	pblk->p_capacity = nr_free_chks * clba;

	printk("provisioned = %llu blk_meta = %d clba = %d\n", provisioned, blk_meta, clba);
	printk("pblk capacity = %llu\n", pblk->capacity);
	printk("pblk p_capacity = %llu\n", pblk->p_capacity);
...
static int pblk_lines_init(struct pblk *pblk)
{
	...
	for (i = 0; i < l_mg->nr_lines; i++) {
		...
		line->wom_total_invalid_cells = 0;
		line->wom_gc = 0;
		line->wom_zero_data = 1;
	}
	...
	printk("%s():calling pblk_set_provision\n", __func__);
		       ...
		       int flags)
{
	...
	ret = pblk_p2l_init(pblk);
	if (ret) {
		pblk_err(pblk, "could not initialize maps\n");
		goto fail_free_rwb;
	}
  
	pblk->num_lines_gced = 0;
