...
//	printk("could not find data bits %x for prev_code %x\n", data_bits, previous_code_bits);
	return 0xFF;
}
...
int encode(uint8_t *data_buf, uint8_t *previous_code_buf, uint8_t *code_buf, int data_size)
{
	...
	{
		if(code_buf[i] != 0x00) {
			printk("caller needs to initialize codebuf to 0x00 at offset %d for codesize %d data_size %d code_bits_per_codeword %d data_bits_per_codeword %d\n", i, code_size, data_size, code_bits_per_codeword, data_bits_per_codeword);
			//assert(0);
		}
	}
	...
	//printk("data_bits_per_codeword %d num_bits %d\n", data_bits_per_codeword, num_bits);
	for(i = (firstCell * data_bits_per_codeword) ; i < num_bits ; i+= data_bits_per_codeword) {
		...
		for (j = 0 ; j < code_bits_per_codeword ; j++)
		{
			...
			if(previous_code_byte_index >= code_size) {
				printk("previous_code_byte_index = %d previous_code_bit_index %d\
		...
		//printk("data_bits %x previous_code_bits %x new code bits %x\n", data_bits, previous_code_bits, code_bits);
