...
void pblk_sysfs_exit(struct gendisk *tdisk)
{
	struct pblk *pblk = tdisk->private_data;
	printk("%s():EXIT\n", __func__);
	printk("%s(): user_wa = %llu gc_wa = %llu pad_wa = %llu num_lines gced=%u\n", __func__, atomic64_read(&pblk->user_wa), atomic64_read(&pblk->gc_wa), atomic64_read(&pblk->pad_wa), pblk->num_lines_gced);
