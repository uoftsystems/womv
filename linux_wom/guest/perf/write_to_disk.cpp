...
int main(int argc, char *argv[])
{
	...
	unsigned long long time_read=0, time_write=0;
	...
	if (argc != 3) {
		printf("Usage: ./write_to_disk <device> <infile>\n");
		exit(0);
	}
	...
	string s(argv[2]);
	s+=".timestamp.wom_code_";
	s+=to_string(wom_code);
	std::ofstream of(s.c_str(), std::ifstream::app);
...
//		printf("real_bno = %llu\n", real_bno);
	...
	//		assert(ret == real_iosize_num_blocks);
			...
			off_t rand_buf_offset = rand() % (randbuf_size - (real_iosize * wom_code) );
